...
SELECT 
    (
        SUM(POWER(age - (SELECT AVG(age) FROM bonus_allocation1), 3)) / 
        (COUNT(*) * POWER((SELECT STDDEV(age) FROM bonus_allocation1), 3))
    ) AS age_skewness
FROM bonus_allocation1;
...
SELECT 
    (
        (SUM(POWER(age - (SELECT AVG(age) FROM bonus_allocation1), 4)) / 
        (COUNT(*) * POWER((SELECT STDDEV(age) FROM bonus_allocation1), 4))) - 3
    ) AS age_kurtosis
FROM bonus_allocation1;
